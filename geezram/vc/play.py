...
from geezram.vc.queues import QUEUE, add_to_queue, get_queue
...
async def ping(_, e: Message):
    ...
    if replied:
        if replied.audio or replied.voice:
            await e.delete()
            izzy = await replied.reply_text("**Transcoding Mp3...**")
        ...
        if len(e.command) < 2:
            ...
            await e.delete()
            izzy = await e.reply_text("__Searching...__")
            query = e.text.split(" ", 1)[1]
            search = ytsearch(query)
            if search == 0:
                await izzy.edit("`Didn't Find Anything for the Given Query`")
            else:
                songname = search[0]
                title = search[0]
                url = search[1]
                ...
                if hm == 0:
                    await izzy.edit(f"**YTDL ERROR ️!** \n\n`{ytlink}`")
                else:
                    if chat_id in QUEUE:
                        pos = add_to_queue(chat_id, songname, ytlink, url, "Audio", 0)
                        await izzy.edit(f"**> Playing in:** {srrf} \n\n**> Song:** {songname} \n**> Position:** #{pos} \n\n**BY:** [{user_name}]({userid})")
                    else:
                        try:
