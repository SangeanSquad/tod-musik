...
async def gen_thumb(thumbnail, title, userid, ctitle):
    async with aiohttp.ClientSession() as session:
        async with session.get(thumbnail) as resp:
            if resp.status == 200:
                f = await aiofiles.open(f"ImagePng/geezram.png", mode="wb")
                await f.write(await resp.read())
                await f.close()
    image1 = Image.open(f"ImagePng/geezram.png")
    image2 = Image.open(f"ImagePng/geezram.png")
    image3 = changeImageSize(1280, 720, image1)
    image4 = changeImageSize(1280, 720, image2)
    image5 = image3.convert("RGBA")
    image6 = image4.convert("RGBA")
    Image.alpha_composite(image5, image6).save(f"ImagePng/geezram.png")
    img = Image.open(f"ImagePng/geezram.png")
    draw = ImageDraw.Draw(img)
    font = ImageFont.truetype("todCore/choose/Roboto-Light.ttf", 55)
    font2 = ImageFont.truetype("todCore/choose/finalfont.ttf", 65)
    draw.text(
        (20, 630),
        f"{title[:25]}...",
        fill="White",
    ...
    img.save(f"ImagePng/geezram.png")
    os.remove(f"ImagePng/geezram.png")
    os.remove(f"ImagePng/geezram.png")
    final = f"ImagePng/geezram.png"
    return final
